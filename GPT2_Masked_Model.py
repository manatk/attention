...
def main():
    ...
    initial_output = outputs.logits
    initial_token_ids = initial_output.argmax(dim=-1).squeeze().tolist()
    initial_decoded_text = tokenizer.decode(initial_token_ids, skip_special_tokens=True)
    #print("Initial Model Output (Decoded):")
    #print(initial_decoded_text)
    ...
    for i in range(0, len(attention_masks)):
        print(attentions[i], attention_masks[i])
    ...
    token_ids = final_output.argmax(dim=-1).squeeze().tolist()
